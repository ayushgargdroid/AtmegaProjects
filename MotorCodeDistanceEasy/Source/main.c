	...
	while(1)
	{
		...
		{//Perfect Y Axis
		    ...
		    {//Both the motors work in co-ordination forward
		      rightf = leftr = 0;
		      leftf = rightr = map(y,0,511,255,0);
		    }
		}
		...
		{//Perfect X Axis
		    ...
		    {//180deg
		      leftf = leftr = 0;
		      rightf = 0;rightr = map(x,0,511,255,0);
		    }
		}
		else
		{
		    ...
		    if(y<512-buffer2)
		    {
		      if(x>512+buffer2)
		      {
		        ...
		        {
		          leftr = 0;
		          leftf = map(y,0,511,255,0);
		          rightf = 0;
		          rightr = map(x,512+buffer2,refx,255,128);
		        }
		        else
		        {
		          refx = x;
		          refy = 1023-refx;
		          leftr = 0;
		          leftf = map(x,512+buffer2,1023,0,255);
		          rightr = 0;
		          rightf = map(y,refy,511,127,0);
		        }
		      }
		      else if(x<512-buffer2)
		      {
				  ...
				  {
					  rightf = 0;
				      rightr = map(y,0,511,255,0);
				      leftr = 0;
				      leftf = map(x,refx,511-buffer2,128,255);
				  }
				  else
				  {
					  refx = refy = x;
				      rightf = 0;
				      rightr = map(x,0,511-buffer2,255,0);
				      leftf = 0;
				      leftr = map(y,refy,511,0,127);
				  }
		      }
		    }
		    else
		    {
		    	if(x<512-buffer2)
		    	{
		    		...
		    		if(x>refx)
		    		{
		    			...
		    			leftf = 0;
		    			leftr = map(x,refx,511-buffer2,0,255);
		    		}
		    		else
		    		{
		    			/*refx = x;
		    	...
		    	else
		    	{
		    		...
		    		else
		    		{
		    			...
		    			rightr = 0;
		    			rightf = map(x,512+buffer2,refx,255,0);
		    		}
		    	}
		    }
...
ISR(ADC_vect)
{
	...
	if(x==prevx && y==prevy)
		k++;
	ADCSRA |= 1<<ADSC;
	if(k>=10000 && !(x<=512+buffer2 && x>=512-buffer2 && y<=512+buffer2 && y>=512-buffer2))
	{
		kill = 1;
		k = 0;
		ADCSRA &= ~(1<<ADSC);
